...
vec4 c = vec4(color * (p.x*colorBlendingGradientX) + (qq.y*addBlurSurface1) + (qq.x*addBlurSurface1)+red, (qq.x*moveContrast) + (p.y*colorBlendingGradientY) * color + (r.y*addInnerSurface1) + (r.y*addInnerSurface2)+green, qq.x*  r.x + (qq.x*fillThreshold)+color+blue,1.);
